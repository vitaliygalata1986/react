...
function App() {
  const [state, formAction, pending] = useActionState(handleSubmit, null); // функция обработчик, начальное состояние // возвращает state, formAction, pending
  ...
  return (
    <>
      ...
      <form action={handleSubmit} className='card shadow p-3'>
        ...
        <button className='btn btn-dark' disabled=>Update</button>
        {error && <p>{error}</p>}
      </form>
    </>
  );
